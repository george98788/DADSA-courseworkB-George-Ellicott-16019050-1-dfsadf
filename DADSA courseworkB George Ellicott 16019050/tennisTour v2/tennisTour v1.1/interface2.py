...
##created 01/11/2017
##revised: 29/11/2017 - added comments
...
def rankingsUpdate(finished):
    place = finished
    placePoints = 0.0
    with open('RANKING UPDATE POINTS.csv', newline='') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
            if place == row['Place']:
                placePoints = row['Tournament Ranking  Points']
...
#then passes selections into function to load correct CSV tournament selections.
#parameters: none
#return type: none
# def tournamentsViaCSV():
...
#         tennisRounds(tournament,mensOrWomens)
...
def tournamentsViaManualInput():
    ...
    if tournamentMenuInput == '1':
        tournament = tournamentNames[0]
        manualInputScores(tournament,mensOrWomens,1)
    elif tournamentMenuInput == '2':
        tournament = tournamentNames[1]
        manualInputScores(tournament,mensOrWomens,1)
    elif tournamentMenuInput == '3':
...
#return type: none
def manualInputScores(tournament,mensOrWomens,TourRound):
    winner = []
    final_loser = []
        ...
        with open('ROUND PROGRESS.csv', 'w', newline='') as csvfile:
            ...
            with open('DADSA 17-18 COURSEWORK B '+tournament+' ROUND '+TournamentRound+' '+mensOrWomens+'.csv', newline='') as csvfile:
                ...
                for row in reader:
                    ...
                    if TournamentRound == 6:
                          if (player_score_a > player_score_b):
                              ...
                              final_loser.append(row['Player B'])
                          else:
                              ...
                              final_loser.append(row['Player A'])
                    if (player_score_a < player_score_b):
                          if (int(player_score_b) - int(player_score_a) == 3)or (mensOrWomens == 'LADIES' and (int(player_score_b) - int(player_score_a) == 2)):
                              if TournamentRound < 6:
                                  bonusWinMaxPoints.append(row['Player B'])
                ...
                for i in doesNotAdvancedToNextRound: # i stands for player A or B
                    ...
                    # basePoints = rankingsUpdate(finished)
                    ...
                    if mensOrWomens == 'MEN':
                        for player in maleLeaderBoard:
                            if i == player._name:
                                ...
                                # player._ranking += totalpoints
                                # player._prizeMoney += prizeMoney
                                print(player._name+str(player._ranking))
                    if mensOrWomens == 'LADIES':
                        for player in femaleLeaderBoard:
                            if i == player._name:
                                totalpoints = totalRoundPoints(tournament,roundpoints)
                ...
                for i in winByOne:
                    if mensOrWomens == 'MEN':
                         for player in maleLeaderBoard:
                             if i == player._name:
                                roundpoints =  rankings(finished,tournament)
                                print(str(roundpoints))
                                thebonus = bonusPoints(roundpoints)
                                thebonus = totalRoundPoints(tournament,thebonus)
                                print(player._name+ "the bonus here is:" + str(thebonus ))
                                player._ranking += thebonus
                                #player._prizeMoney += prizeMoney
                                ...
                                if TournamentRound == 6:
                                     player._ranking += thebonus
                                     print("remove this: " +str(thebonus))
                                     player._ranking =  player._ranking - thebonus
                                     break
                    ...
                    if mensOrWomens == 'MEN':
                        for player in maleLeaderBoard:
                            if i == player._name:
                                ...
                                print(player._name+ "second::" + str(thebonus ))
                                player._ranking += thebonus
            ...
            menu()
             ...
             with open('ROUND PROGRESS.csv', newline='') as csvfile:
                    ...
                    for row in reader:
                        ...
                        TournamentRound = int(TournamentRound)
                        TournamentRound = (TournamentRound+1)
                        manualInputScores(tournament,mensOrWomens,TournamentRound)
        elif menuInput == '0':
...
def percentageCal(playerWins, playerlosses):
    if (playerWins + playerlosses) == 0.0:
        percentageOfWins = 0.0
    else:
